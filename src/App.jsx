      ...
      {tab === 'menutypes' && (
        <div style={{padding:20}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
            ...
            <button onClick={()=>setSheet('newRecipe')} style={{background:P.accent, color:'white', border:'none', padding:'8px 16px', borderRadius:12}}>+ Nouveau</button>
          </div>
          {recipes.map(r => (
            <div key={r.id} style={{padding:12, background:P.surface, borderRadius:12, marginBottom:8, border:`1px solid ${P.border}`, display:'flex', justifyContent:'space-between'}}>
              <span>{r.protein} {r.name}</span>
